...
volatile UINT8 accelParamsUpdated = 0;
...
volatile Matlab_GUI matlab_gui;
...
interrupt 38 void CANRx_vect(void)
{
     ...
     if(identifier == CAN_PARAM_MSG_ID)
    {
        if(!carParamsUpdated) // Only update when old values have been used
        {
            ...
            memcpy(&(matlab_gui.carParams), &CANRXDSR0, length);
            carParamsUpdated = 1;
        }
    }
    ...
    else if(identifier == CAN_BRAKE_MSG_ID)
    {
        //if(!brakeParamsUpdated) // Only update when old values have been used
        {
            ...
            memcpy(&(matlab_gui.Brakeparams), &CANRXDSR0, length);
    ...
    { 
        {
            ...
            memcpy(&(matlab_gui.accelparams), &CANRXDSR0, length);
            accelParamsUpdated = 1;
        }
    }
    ...
    { 
        {
            ...
            memcpy(&(matlab_gui.desired_speed), &CANRXDSR0, length);
...
void main(void)
{
    ...
    for(;;)
    {
        ...
        if(verifyMsgUpdated || taskfeedbackUpdated || cruisespeedUpdated || brakeParamsUpdated || accelParamsUpdated) 
        {
