{ =====================================================================
Register and vector equates for MSP430x16x

Copyright (C) 2003  FORTH, Inc.

Standard register, bit, and interrupt vector definitions for the
Texas Instruments MSP430 microcontroller.

===================================================================== }

ASSEMBLER

\ ***********************************************************
\  STATUS REGISTER BITS
\ ***********************************************************

$0001 CONSTANT C
$0002 CONSTANT Z
$0004 CONSTANT N
$0100 CONSTANT V
$0008 CONSTANT GIE
$0010 CONSTANT CPUOFF
$0020 CONSTANT OSCOFF
$0040 CONSTANT SCG0
$0080 CONSTANT SCG1

\ Low Power Modes coded with Bits 4-7 in SR.  See "OPERATING MODES"
\ section of the MSP430x16x User's Guide for details about LPM.

0                               EQU AM          \ Active Mode
CPUOFF                          EQU LPM0        \ Low Power Mode 0
SCG0 CPUOFF +                   EQU LPM1        \ etc...
SCG1 CPUOFF +                   EQU LPM2
SCG1 SCG0 CPUOFF + +            EQU LPM3
SCG1 SCG0 OSCOFF CPUOFF + + +   EQU LPM4

SCG1 SCG0 OSCOFF CPUOFF + + +   EQU &LPM        \ LPM bit mask

TARGET

\ ***********************************************************
\  SPECIAL FUNCTION REGISTER ADDRESSES + CONTROL BITS
\ **********************************************************

$0000 EQU IE1
$0000 EQU U0IE
$01 EQU WDTIE
$02 EQU OFIE
$10 EQU NMIIE
$20 EQU ACCVIE
$40 EQU URXIE0
$80 EQU UTXIE0

$0002 EQU IFG1
$0002 EQU U0IFG
$01 EQU WDTIFG
$02 EQU OFIFG
$10 EQU NMIIFG
$40 EQU URXIFG0
$80 EQU UTXIFG0

$0004 EQU ME1
$0004 EQU U0ME
$40 EQU URXE0
$40 EQU USPIE0
$80 EQU UTXE0

$0001 EQU IE2
$0001 EQU U1IE
$10 EQU URXIE1
$20 EQU UTXIE1

$0003 EQU IFG2
$0003 EQU U1IFG
$10 EQU URXIFG1
$20 EQU UTXIFG1

$0005 EQU ME2
$0005 EQU U1ME
$10 EQU URXE1
$10 EQU USPIE1
$20 EQU UTXE1

\ ***********************************************************
\  WATCHDOG TIMER
\ **********************************************************

$0120 EQU WDTCTL

$0001 EQU WDTIS0
$0002 EQU WDTIS1
$0004 EQU WDTSSEL
$0008 EQU WDTCNTCL
$0010 EQU WDTTMSEL
$0020 EQU WDTNMI
$0040 EQU WDTNMIES
$0080 EQU WDTHOLD

$5A00 EQU WDTPW

\ ***********************************************************
\  HARDWARE MULTIPLIER
\ **********************************************************

$0130 EQU MPY
$0132 EQU MPYS
$0134 EQU MAC
$0136 EQU MACS
$0138 EQU OP2
$013A EQU RESLO
$013C EQU RESHI
$013E EQU SUMEXT

\ ***********************************************************
\  DIGITAL I/O Port1/2
\ **********************************************************

$0020 EQU P1IN
$0021 EQU P1OUT
$0022 EQU P1DIR
$0023 EQU P1IFG
$0024 EQU P1IES
$0025 EQU P1IE
$0026 EQU P1SEL

$0028 EQU P2IN
$0029 EQU P2OUT
$002A EQU P2DIR
$002B EQU P2IFG
$002C EQU P2IES
$002D EQU P2IE
$002E EQU P2SEL

\ ***********************************************************
\  DIGITAL I/O Port3/4
\ **********************************************************

$0018 EQU P3IN
$0019 EQU P3OUT
$001A EQU P3DIR
$001B EQU P3SEL

$001C EQU P4IN
$001D EQU P4OUT
$001E EQU P4DIR
$001F EQU P4SEL

\ ***********************************************************
\  DIGITAL I/O Port5/6
\ **********************************************************

$0030 EQU P5IN
$0031 EQU P5OUT
$0032 EQU P5DIR
$0033 EQU P5SEL

$0034 EQU P6IN
$0035 EQU P6OUT
$0036 EQU P6DIR
$0037 EQU P6SEL

\ ***********************************************************
\  USART
\ **********************************************************

$80 EQU PENA
$40 EQU PEV
$20 EQU SPB
$10 EQU CHARB           \ Renamed - conflicts with standard word CHAR
$08 EQU LISTEN
$04 EQU SYNC
$02 EQU MM
$01 EQU SWRST

$80 EQU CKPH
$40 EQU CKPL
$20 EQU SSEL1
$10 EQU SSEL0
$08 EQU URXSE
$04 EQU TXWAKE
$02 EQU STC
$01 EQU TXEPT

$80 EQU FE
$40 EQU PE
$20 EQU OE
$10 EQU BRK
$08 EQU URXEIE
$04 EQU URXWIE
$02 EQU RXWAKE
$01 EQU RXERR

\ ***********************************************************
\  USART 0
\ **********************************************************

$0070 EQU U0CTL
$0071 EQU U0TCTL
$0072 EQU U0RCTL
$0073 EQU U0MCTL
$0074 EQU U0BR0
$0075 EQU U0BR1
$0076 EQU U0RXBUF
$0077 EQU U0TXBUF

\ ***********************************************************
\  USART 1
\ **********************************************************

$0078 EQU U1CTL
$0079 EQU U1TCTL
$007A EQU U1RCTL
$007B EQU U1MCTL
$007C EQU U1BR0
$007D EQU U1BR1
$007E EQU U1RXBUF
$007F EQU U1TXBUF

\ ***********************************************************
\  I2C
\ **********************************************************

\ In USART 0 Control (U0CTL)
$01 EQU I2CEN
$02 EQU MST
\ $04 EQU SYNC          same as above
\ $08 EQU LISTEN        same as above
$10 EQU XA
$20 EQU I2C
$40 EQU TXDMAEN
$80 EQU RXDMAEN

$0050 EQU I2CIE
$01 EQU ALIE
$02 EQU NACKIE
$04 EQU OAIE
$08 EQU ARDYIE
$10 EQU RXRDYIE
$20 EQU TXRDYIE
$40 EQU GCIE
$80 EQU STTIE

$0051 EQU I2CIFG
$01 EQU ALIFG
$02 EQU NACKIFG
$04 EQU OAIFG
$08 EQU ARDYIFG
$10 EQU RXRDYIFG
$20 EQU TXRDYIFG
$40 EQU GCIFG
$80 EQU STTIFG

$0052 EQU I2CNDAT

$0071 EQU I2CTCTL
$01 EQU I2CSTT
$02 EQU I2CSTP
$04 EQU I2CSTB
$08 EQU I2CTRX
$10 EQU I2CSSEL0
$20 EQU I2CSSEL1
$40 EQU I2CRM
$80 EQU I2CWORD

$0072 EQU I2CDCTL
$01 EQU I2CBB
$02 EQU I2CRXOVR
$04 EQU I2CTXUDF
$08 EQU I2CSBD
$10 EQU I2CSCLLOW

$0073 EQU I2CPSC
$0074 EQU I2CSCLH
$0075 EQU I2CSCLL
$0076 EQU I2CDR

$0118 EQU I2COA
$011A EQU I2CSA

$011C EQU I2CIV
$0000 EQU I2CIV_NONE
$0002 EQU I2CIV_AL
$0004 EQU I2CIV_NACK
$0006 EQU I2CIV_OA
$0008 EQU I2CIV_ARDY
$000A EQU I2CIV_RXRDY
$000C EQU I2CIV_TXRDY
$000E EQU I2CIV_GC
$0010 EQU I2CIV_STT

\ ***********************************************************
\  Timer A
\ **********************************************************

$012E EQU TAIV
$0160 EQU TACTL
$0162 EQU TACCTL0
$0164 EQU TACCTL1
$0166 EQU TACCTL2
$0170 EQU TAR
$0172 EQU TACCR0
$0174 EQU TACCR1
$0176 EQU TACCR2

\  Alternate register names

$0162 EQU CCTL0
$0164 EQU CCTL1
$0166 EQU CCTL2
$0172 EQU CCR0
$0174 EQU CCR1
$0176 EQU CCR2

$0400 EQU TASSEL2
$0200 EQU TASSEL1
$0100 EQU TASSEL0
$0080 EQU ID1
$0040 EQU ID0
$0020 EQU MC1
$0010 EQU MC0
$0004 EQU TACLR
$0002 EQU TAIE
$0001 EQU TAIFG

$8000 EQU CM1
$4000 EQU CM0
$2000 EQU CCIS1
$1000 EQU CCIS0
$0800 EQU SCS
$0400 EQU SCCI
$0100 EQU CAP
$0080 EQU OUTMOD2
$0040 EQU OUTMOD1
$0020 EQU OUTMOD0
$0010 EQU CCIE
$0008 EQU CCI
$0004 EQU OUT
$0002 EQU COV
$0001 EQU CCIFG

\ ***********************************************************
\  Timer B
\ **********************************************************

$011E EQU TBIV
$0180 EQU TBCTL
$0182 EQU TBCCTL0
$0184 EQU TBCCTL1
$0186 EQU TBCCTL2
$0188 EQU TBCCTL3
$018A EQU TBCCTL4
$018C EQU TBCCTL5
$018E EQU TBCCTL6
$0190 EQU TBR
$0192 EQU TBCCR0
$0194 EQU TBCCR1
$0196 EQU TBCCR2
$0198 EQU TBCCR3
$019A EQU TBCCR4
$019C EQU TBCCR5
$019E EQU TBCCR6

$4000 EQU SHR1
$2000 EQU SHR0
$4000 EQU TBCLGRP1
$2000 EQU TBCLGRP0
$1000 EQU CNTL1
$0800 EQU CNTL0
$0400 EQU TBSSEL2
$0200 EQU TBSSEL1
$0100 EQU TBSSEL0
$0004 EQU TBCLR
$0002 EQU TBIE
$0001 EQU TBIFG

$0400 EQU SLSHR1
$0200 EQU SLSHR0
$0400 EQU CLLD1
$0200 EQU CLLD0

\ ***********************************************************
\  Basic Clock Module
\ **********************************************************

$0056 EQU DCOCTL
$0057 EQU BCSCTL1
$0058 EQU BCSCTL2

$01 EQU MOD0
$02 EQU MOD1
$04 EQU MOD2
$08 EQU MOD3
$10 EQU MOD4
$20 EQU DCO0
$40 EQU DCO1
$80 EQU DCO2

$01 EQU RSEL0
$02 EQU RSEL1
$04 EQU RSEL2
$08 EQU XT5V
$10 EQU DIVA0
$20 EQU DIVA1
$40 EQU XTS
$80 EQU XT2OFF

$01 EQU DCOR
$02 EQU DIVS0
$04 EQU DIVS1
$08 EQU SELS
$10 EQU DIVM0
$20 EQU DIVM1
$40 EQU SELM0
$80 EQU SELM1

\ ************************************************************
\  Flash Memory
\ ***********************************************************

$0128 EQU FCTL1
$012A EQU FCTL2
$012C EQU FCTL3

$9600 EQU FRKEY
$A500 EQU FWKEY
$3300 EQU FXKEY

$0002 EQU ERAS          \ Renamed -- conflicts with standard word ERASE
$0004 EQU MERAS
$0040 EQU WRT
$0080 EQU BLKWRT
$0080 EQU SEGWRT

$0001 EQU FN0
$0002 EQU FN1
$0004 EQU FN2
$0008 EQU FN3
$0010 EQU FN4
$0020 EQU FN5
$0040 EQU FSSEL0
$0080 EQU FSSEL1

$0001 EQU BUSY
$0002 EQU KEYV
$0004 EQU ACCVIFG
$0008 EQU FWAIT         \ Renamed -- conflicts with Multitasker WAIT
$0010 EQU LOCK
$0020 EQU EMEX

\ ***********************************************************
\  Comparator A
\ **********************************************************

$0059 EQU CACTL1
$005A EQU CACTL2
$005B EQU CAPD

$01 EQU CAIFG
$02 EQU CAIE
$04 EQU CAIES
$08 EQU CAON
$10 EQU CAREF0
$20 EQU CAREF1
$40 EQU CARSEL
$80 EQU CAEX

$01 EQU CAOUT
$02 EQU CAF
$04 EQU P2CA0
$08 EQU P2CA1
$10 EQU CACTL24
$20 EQU CACTL25
$40 EQU CACTL26
$80 EQU CACTL27

$01 EQU CAPD0
$02 EQU CAPD1
$04 EQU CAPD2
$08 EQU CAPD3
$10 EQU CAPD4
$20 EQU CAPD5
$40 EQU CAPD6
$80 EQU CAPD7

\ ***********************************************************
\  ADC12
\ **********************************************************

$01A0 EQU ADC12CTL0
$01A2 EQU ADC12CTL1
$01A4 EQU ADC12IFG
$01A6 EQU ADC12IE
$01A8 EQU ADC12IV

$0140 EQU ADC12MEM0
$0142 EQU ADC12MEM1
$0144 EQU ADC12MEM2
$0146 EQU ADC12MEM3
$0148 EQU ADC12MEM4
$014A EQU ADC12MEM5
$014C EQU ADC12MEM6
$014E EQU ADC12MEM7
$0150 EQU ADC12MEM8
$0152 EQU ADC12MEM9
$0154 EQU ADC12MEM10
$0156 EQU ADC12MEM11
$0158 EQU ADC12MEM12
$015A EQU ADC12MEM13
$015C EQU ADC12MEM14
$015E EQU ADC12MEM15

$0080 EQU ADC12MCTL0
$0081 EQU ADC12MCTL1
$0082 EQU ADC12MCTL2
$0083 EQU ADC12MCTL3
$0084 EQU ADC12MCTL4
$0085 EQU ADC12MCTL5
$0086 EQU ADC12MCTL6
$0087 EQU ADC12MCTL7
$0088 EQU ADC12MCTL8
$0089 EQU ADC12MCTL9
$008A EQU ADC12MCTL10
$008B EQU ADC12MCTL11
$008C EQU ADC12MCTL12
$008D EQU ADC12MCTL13
$008E EQU ADC12MCTL14
$008F EQU ADC12MCTL15

$001 EQU ADC12SC
$002 EQU ENC
$004 EQU ADC12TOVIE
$008 EQU ADC12OVIE
$010 EQU ADC12ON
$020 EQU REFON
$040 EQU REF2_5V
$080 EQU MSH
$080 EQU MSC

$0001 EQU ADC12BUSY

0 EQU INCH_0
1 EQU INCH_1
2 EQU INCH_2
3 EQU INCH_3
4 EQU INCH_4
5 EQU INCH_5
6 EQU INCH_6
7 EQU INCH_7
8 EQU INCH_8
9 EQU INCH_9
10 EQU INCH_10
11 EQU INCH_11
12 EQU INCH_12
13 EQU INCH_13
14 EQU INCH_14
15 EQU INCH_15

$80 EQU EOS

\ ***********************************************************
\  DAC12
\ **********************************************************

$01C0 EQU DAC12_0CTL
$01C2 EQU DAC12_1CTL

$0001 EQU DAC12GRP
$0002 EQU DAC12ENC
$0004 EQU DAC12IFG
$0008 EQU DAC12IE
$0010 EQU DAC12DF
$0020 EQU DAC12AMP0
$0040 EQU DAC12AMP1
$0080 EQU DAC12AMP2
$0100 EQU DAC12IR
$0200 EQU DAC12CALON
$0400 EQU DAC12LSEL0
$0800 EQU DAC12LSEL1
$1000 EQU DAC12RES
$2000 EQU DAC12SREF0
$4000 EQU DAC12SREF1

$01C8 EQU DAC12_0DAT
$01CA EQU DAC12_1DAT

\ ***********************************************************
\  DMA
\ **********************************************************

$0122 EQU DMACTL0
$0001 EQU DMA0TSEL0
$0002 EQU DMA0TSEL1
$0004 EQU DMA0TSEL2
$0008 EQU DMA0TSEL3
$0010 EQU DMA1TSEL0
$0020 EQU DMA1TSEL1
$0040 EQU DMA1TSEL2
$0080 EQU DMA1TSEL3
$0100 EQU DMA2TSEL0
$0200 EQU DMA2TSEL1
$0400 EQU DMA2TSEL2
$0800 EQU DMA2TSEL3

$0124 EQU DMACTL1
$0001 EQU ENNMI
$0002 EQU ROUNDROBIN
$0004 EQU DMAONFETCH

$01E0 EQU DMA0CTL
$01E8 EQU DMA1CTL
$01F0 EQU DMA2CTL

$0001 EQU DMAREQ
$0002 EQU DMAABORT
$0004 EQU DMAIE
$0008 EQU DMAIFG
$0010 EQU DMAEN
$0020 EQU DMALEVEL
$0040 EQU DMASRCBYTE
$0080 EQU DMADSTBYTE
$0100 EQU DMASRCINCR0
$0200 EQU DMASRCINCR1
$0400 EQU DMADSTINCR0
$0800 EQU DMADSTINCR1
$1000 EQU DMADT0
$2000 EQU DMADT1
$4000 EQU DMADT2

$01E2 EQU DMA0SA
$01E4 EQU DMA0DA
$01E6 EQU DMA0SZ
$01EA EQU DMA1SA
$01EC EQU DMA1DA
$01EE EQU DMA1SZ
$01F2 EQU DMA2SA
$01F4 EQU DMA2DA
$01F6 EQU DMA2SZ

\ ***********************************************************
\  Interrupt Vectors
\ **********************************************************

$FFE0 EQU DACDMA_VECTOR
$FFE2 EQU PORT2_VECTOR
$FFE4 EQU USART1TX_VECTOR
$FFE6 EQU USART1RX_VECTOR
$FFE4 EQU UART1TX_VECTOR        \ Compatibility
$FFE6 EQU UART1RX_VECTOR        \ Compatibility
$FFE8 EQU PORT1_VECTOR
$FFEA EQU TIMERA1_VECTOR
$FFEC EQU TIMERA0_VECTOR
$FFEE EQU ADC_VECTOR
$FFF0 EQU USART0TX_VECTOR
$FFF2 EQU USART0RX_VECTOR
$FFF0 EQU UART0TX_VECTOR        \ Compatibility
$FFF2 EQU UART0RX_VECTOR        \ Compatibility
$FFF4 EQU WDT_VECTOR
$FFF6 EQU COMPARATORA_VECTOR
$FFF8 EQU TIMERB1_VECTOR
$FFFA EQU TIMERB0_VECTOR
$FFFC EQU NMI_VECTOR
$FFFE EQU RESET_VECTOR
