...
// token timeout in ms
#define RTL_TOKEN_TIMEOUT	 40000  //fix srinivas
...
extern nrk_task_type rtl_task;
extern NRK_STK rtl_task_stack[RT_LINK_STACK_SIZE];
...
extern rtl_node_mode_t rtl_node_mode;


// some handy book keeping values
extern uint16_t last_slot;
extern nrk_time_t last_slot_time;
...
extern uint8_t _rtl_sync_ok;
extern volatile uint8_t _rtl_ready;
...
extern uint8_t _rtl_time_token;
extern uint8_t _rtl_time_token_status;


extern RF_TX_INFO rtl_tsync_tx;
extern uint8_t rtl_tsync_buf[PKT_DATA_START];

extern uint8_t _rtl_contention_pending;  //fix
extern uint8_t _rtl_contention_slots; //fix


extern RF_RX_INFO rtl_rfRxInfo;
//extern uint8_t rtl_rx_data_ready;
extern volatile uint8_t rtl_rx_slot;
// This is the link layer's only buffer
extern volatile uint8_t rtl_rx_buf[RF_MAX_PAYLOAD_SIZE];  

// Extra slot at end for abs slot buffer pointer  
extern volatile RF_TX_INFO rtl_rfTxInfo;
...
extern RTL_TX_INFO	rtl_tx_info[TDMA_FRAME_SLOTS+1]; 

extern uint16_t rtl_abs_tx_slot;
extern uint8_t rtl_abs_tx_ready;
extern uint32_t rtl_tx_data_ready;
...
// 5 -> 16
// 6 -> 32
extern uint32_t rtl_tdma_rx_mask;
extern uint32_t rtl_tdma_tx_mask;        


// Event Callback functions
extern void (*rx_callback)(uint8_t slot);
extern void (*tx_callback)(uint8_t slot);
extern void (*abs_callback)(uint16_t global_slot);
extern void (*slot_callback)(uint16_t global_slot);
extern void (*cycle_callback)(uint16_t global_cycle); // cycles since the epoch
