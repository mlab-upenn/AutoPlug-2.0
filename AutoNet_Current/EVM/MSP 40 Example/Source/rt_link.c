...
void (*cycle_callback)(uint16_t global_cycle);
...
*/
    ...
    _nrk_set_next_wakeup(2000);//_nrk_set_next_wakeup(250);//fix srinivas
    //timeout=200;
	...
	if(last_nrk_tick!=_nrk_os_timer_get()) 
		...
		if(last_nrk_tick>=_nrk_get_next_wakeup()-2) _nrk_os_timer_set(0);//fix srinivas
	   	timer++;
    ...
    // reset OS timer
    _nrk_os_timer_set(0);//fix srinivas
    ...
    timeout = tdma_start_tick+16;//16;//+4; //fix srinivas
...
//    printf( "tt=%d et=%d %d\r\n",tmp_token,elapsed_time,tick_to_slot_offset );	
    ...
    //printf( "sfd-start = %d %d\r\n",sfd_start_time,_nrk_high_speed_timer_get() );
     _nrk_os_timer_stop();
     _nrk_os_timer_reset();
     _nrk_os_timer_set(7);//_nrk_os_timer_set(7);//fix srinivas
    ...
    //_nrk_set_next_wakeup(5);  //fix srinivas
...
void _rtl_rx (uint8_t slot)
{
    ...
    timeout+=16;//16;//timeout+=4; //rtl_param.rx_timeout;//fix srinivas
     n = 0;
    ...
    timeout = _nrk_os_timer_get(); 
    timeout += 20;//20;//timeout += 20;//timeout += 5;               // was 30 //fix srinivas
...
// Set next wakeup
		...
		if(slot_mask & rtl_tdma_tx_mask )  // Yes it is your TX slot
			...
			if((slot_mask & rtl_tx_data_ready) == 0)  // No app data to send
				...
				if(slot>(TDMA_FRAME_SLOTS-_rtl_contention_slots))
				{
					...
					{
						//printf( "cs" );
						rtl_tx_pkt (rtl_tsync_tx.pPayload,rtl_tsync_tx.length, RTL_CONTENTION);
					} 
					//else printf( "already pending\r\n" );
				} else
					{
			        	rtl_tx_pkt (rtl_tsync_tx.pPayload,rtl_tsync_tx.length, slot);
					//printf( "ss" );
					}
				}
	...
	// if RX slot mask and RX buffer free, try to receive a packet
        ...
        else if (global_slot == rtl_abs_tx_slot && rtl_abs_tx_ready == 1) {
...
//	printf( "s %d nw %d ",global_slot,next_slot_offset );
	...
	{
        global_slot++;
        nrk_wait_until_ticks(19);//nrk_wait_until_ticks(5); //fix srinivas
	}
	else
	{
...
void rtl_task_config ()
{
    ...
    nrk_task_set_entry_function( &rtl_task, rtl_nw_task);
    nrk_task_set_stk( &rtl_task, rtl_task_stack, RT_LINK_STACK_SIZE);
    rtl_task.prio = 20;
    rtl_task.FirstActivation = TRUE;
    rtl_task.Type = BASIC_TASK;
    rtl_task.SchType = NONPREEMPTIVE;
    rtl_task.period.secs = 0;
    rtl_task.period.nano_secs = RTL_NRK_TICKS_PER_SLOT * NANOS_PER_TICK;//fix srinivas
